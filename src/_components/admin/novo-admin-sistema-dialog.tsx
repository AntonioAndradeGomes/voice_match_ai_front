...
import { criarAdminDoSistema } from "@/lib/empresas";

interface NovoAdminSistemaDialogProps {
    open: boolean;
...
export function NovoAdminSistemaDialog({
    open,
    onOpenChange,
    onAdminCriado,
}: NovoAdminSistemaDialogProps) {
    ...
    async function handleSalvar() {
        setSalvando(true);
        try {
            const criado = await criarAdminDoSistema({
                nome_completo: nome.trim(),
                email: email.trim(),
                senha,
            });
            toast.success(`${criado.nomeCompleto} agora opera a plataforma.`);
            setNome("");
            setEmail("");
            setSenha("");
            onAdminCriado();
            ...
            toast.error(
                erro instanceof Error
                    ? erro.message
                    : "Não foi possível cadastrar o administrador do sistema.",
            );
        } finally {
            setSalvando(false);
        }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Novo administrador do sistema</DialogTitle>
                    <DialogDescription>
                        Esta pessoa passa a cadastrar, suspender e enxergar
                        todas as empresas da plataforma. Não é o administrador
                        de uma empresa cliente — esse se cadastra dentro da
                        empresa.
                    </DialogDescription>
                </DialogHeader>
