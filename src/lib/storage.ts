...
export async function getCandidatosByVaga(vagaId: string): Promise<Candidato[]> {
    if (isValidUUID(vagaId)) {
        try {
            const response = await fetch(`http://localhost:8000/candidaturas/vaga/${vagaId}`);
            if (response.ok) {
                ...
                if (Array.isArray(candidaturas)) {
                    ...
                    for (const cand of candidaturas) {
                        try {
                            const resCand = await fetch(`http://localhost:8000/candidatos/${cand.candidato_id}`);
                            if (resCand.ok) {
...
export async function saveCandidato(candidato: Candidato): Promise<Candidato> {
    if (isValidUUID(candidato.vagaId)) {
        try {
            ...
            const resCandidato = await fetch("http://localhost:8000/candidatos", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    nome: candidato.nome,
            ...
            if (resCandidato.ok) {
                ...
                const listRes = await fetch("http://localhost:8000/candidatos");
                if (listRes.ok) {
            ...
            const resCandidatura = await fetch("http://localhost:8000/candidaturas", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
